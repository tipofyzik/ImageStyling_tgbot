# Import file with architecture and functions for image processing
from MSGNet import *
...
def firstkeyboard():
   button=[
      ...
      types.KeyboardButton(text="/upload"),
...
Дабы узнать функционал и пару тонкостей пользования мной, воспользуйся командой /help.
Дабы перейти к загрузке фото, воспользуйся командой /upload.
...
async def get_images(message: types.Message):
   text=f"""/upload - позволяет перейти к загрузке фотографий.
/original\_image - загружает фото, _на которое_ будет переноситься стиль.
/style\_image - загружает фото, _c которого_ будет браться стиль.
/result - в случае загруки обеих фотографий позволяет увидеть результат.
/restart - если хотите начать всё заново. Команда доступна всегда.

/tests - тут показаны тесты разработчика. Нажми или введи /test, чтобы узнать больше.
...
async def result(message: types.Message):
   ...
   if x is None and y is None:
      await message.answer("Не было получено ни одного фото!")
   elif x is None:
      await message.answer("Оригинал не был получен!")
   elif y is None:
      await message.answer("Стиль не был получен!")
   else:
...
Первая - черепаха - на неё переносился стиль второй фотографии - заката.""", reply_markup=firstkeyboard())
   ...
   media_group.attach_photo(types.InputFile('tests/turtle.jpeg'))
   media_group.attach_photo(types.InputFile('tests/real_sunset.jpg'))
...
Можно легко заметить изменения даже в сжатых файлах, если просто полистать фотографии""", reply_markup=firstkeyboard())
   media_group = types.MediaGroup() 
   media_group.attach_photo(types.InputFile('tests/512.jpg'))
   media_group.attach_photo(types.InputFile('tests/768.jpg'))
   media_group.attach_photo(types.InputFile('tests/1024.jpg'))
   media_group.attach_photo(types.InputFile('tests/1536.jpg'))
   media_group.attach_photo(types.InputFile('tests/2048.jpg'))
   await bot.send_media_group(chat_id=message.chat.id, media=media_group)

   await message.answer("""
image_size=2048 заставил мой компьютер чутка поглючить, а с image_size=4096 вообще не справился.
Для оптимизации времени ожидания image_size был выставлен равным 768, ибо качество не такое плохое, как с 512, а времени съедает не очень много. 
