...
При написании бота возникли некоторые ошибки связанные с тем, что сама модель была реализована более шести лет назад и некоторые параметры на текущий момент конфликтуют. В конце этого раздела будут даны ссылки на решения для каждой проблемы. Отмечены они будут как в ноутбуке "Ошибка [номер]".
