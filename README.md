...
6. В файле tgbot.py есть функция transform_image() - она тоже должна быть асинхронной, как и другие функции. Иными словами, при обработке изображения от пользователя, другой юзер не будет получать сообщения до получения результата первого пользователя.

**UPD: 11.07.2023**
Исправлен 5-й пункт и частично 6-й. Все изменения были в файле "tgbot.py"
5* Исправлен недочёт с перезаписыванием одних и тех же файлов. Теперь бот фотографии каждого юзера будет хранить в отдельной папке, имеющей название никнейма в телеграме. Это позволить каждому пользователю работать лишь со своими фото, не болясь, что что-то пойдёт не так. Всё, что использовалось для решения этой проблемы - функция message.from_user.username, выдающая ник пользователя. Достаточно было переписать пути загрузки и выгрузки фотографий. И теперь удобнее удалять фотографии при вызове /restart - просто удаляется папка с фото при условии её существования.
6* Была исправлена проблема с асинхронностью - надо было при определении функции transform_image() перед ней написать async, а при её вызове надо написать await (функция result, условие "else", вторая строка). Если запускать с компьютера - асинхронность работает.
**НО:** 
1. Несмотря на то, что при запуске бота с железа асинхронность присутствует, результат при запуске через docker/сервер противоположный - асинхронности нет. Бот всё также работает, однако сообщения обрабатываются по порядку - будто исправлений никаких и не было. Буду честен, что не так - не смог разобрать. Асинхронность пробовал реализовать в первый раз - получило, но коряво. В будущих обновлениях постараюсь решить проблему.
2. В боте реализована логика, предназначенная для работы с одним юзером. А именно, при запуске /restart или /start переменные, используемые в логике, "обнуляются", что приводит к багу. 

**Дабы не быть голословным, расскажу на примере:**  
Допустим, user1 загрузил все фотографии и хочет узнать результат переноса стиля. Если user2 перед этим нажмёт /restart, то несмотря на то, что фотографии первого пользователя уже загружены, бот будет говорить, что фотографии не загрузились. Тут нужно доработать логику и сделать акцент на пользователе. Но данный косяк будет исправляться уже в будущем. 
