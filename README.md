...
6. В файле tgbot.py есть функция transform_image() - она тоже должна быть асинхронной. Иными словами, при обработке изображения от пользователя, другой юзер не будет получать сообщения до получения результата первого пользователя.
