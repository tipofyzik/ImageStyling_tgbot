...
async def get_images(message: types.Message):
   await message.answer("""Можешь приступить к загрузке фотографий, не забудь указать тип фотографии - original или style! 
Для этого нажми на соответствующую кнопку или введи команду вручную. Загружать фото можно в любом порядке, главное - загрузить обе.""", reply_markup=keyboard())
...
Для оптимизации времени обработки image_size был выставлен равным 512 - изображения обрабатываются шустро и результат не заставляет себя ждать.
Однако если хочется повысить качество переноса - рекомендую выставить размер равный 1024 или 1536. Дальнейшее увеличение размера улучшает качество, но несущественно.
